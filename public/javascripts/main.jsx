var pages = [{title:"Correlation != Causation", text:"Correlation != Causation is an application highlighting the age old rule of never reading too much into the relationship of two graphs. It randomly scourges through the World Bank database of indicators until it finds two time series that correlate with a coefficient of 0.8 or more. Simple? Very.", buttontext:"This here button sets off the adventure. Brace yourself, it's going to take a while."}]
...
var Nextbutton = React.createClass({
  ...
  handleClick: function(){
    ...
    fetch('/test',{headers:{'Content-Type':'application/json'}}).then(function(response){
      response.json().then(function(ret){
        ...
        var options = {low:0, fullWidth: true, chartPadding: {right: 40}};
        console.log(data);
          ...
          success: function(res){
            ...
            $.ajax({
              ...
              success: function(res){
                ...
                ReactDOM.render(<ChartHolder data={data} options={options} title="Cool stuff" country1={country1} indicator1={indicator1} country2={country2} indicator2={indicator2}/>, document.getElementById('app'));
              }
            });
          }
        });
...
var ChartHolder = React.createClass({
  render: function(){
   return(
    ...
    <p className="result-p country-1">{this.props.indicator1} in {this.props.country1}</p>
    <p className="result-p divider">correlates with</p>
    <p className="result-p country-2">{this.props.indicator2} in {this.props.country2}</p>
    </div>
    )
  }
});
